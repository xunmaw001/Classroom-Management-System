    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kaoqin !=null){
            ...
            BeanUtils.copyProperties( kaoqin , view );//把实体数据重构到view中
                ...
                //级联表
                JiaoshiEntity jiaoshi = jiaoshiService.selectById(kaoqin.getJiaoshiId());
                if(jiaoshi != null){
                    BeanUtils.copyProperties( jiaoshi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setJiaoshiId(jiaoshi.getId());
                }
                //级联表
                KechengEntity kecheng = kechengService.selectById(kaoqin.getKechengId());
                if(kecheng != null){
    ...
    */
    ...
    public R save(@RequestBody KaoqinEntity kaoqin, HttpServletRequest request){
        ...
        if(false)
            ...
            kaoqin.setJiaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<KaoqinEntity> queryWrapper = new EntityWrapper<KaoqinEntity>()
            .eq("kaoqin_uuid_number", kaoqin.getKaoqinUuidNumber())
            .eq("jiaoshi_id", kaoqin.getJiaoshiId())
            .eq("kecheng_id", kaoqin.getKechengId())
            ;
...
//                            kaoqinEntity.setJiaoshiId(Integer.valueOf(data.get(0)));   //教师 要改的
//                            kaoqinEntity.setKechengId(Integer.valueOf(data.get(0)));   //课程 要改的
